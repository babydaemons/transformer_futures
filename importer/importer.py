...
# --- 定数定義 ---
JPX_TSV_COLS = ["trade_date", "time", "trade_price", "price_type", "trade_volume", "contract_month"]
MT5_TSV_COLS = ["timestamp", "bid", "ask"]

class ConvertNK225:
    """JPX先物データの変換ロジックを管理するクラス"""

    @staticmethod
    def read_minimal_tsv(tsv_path: str) -> pl.DataFrame:
        """JPX生TSVを読み込み、型変換とNaiveな日時結合を行う"""
        ...
        if missing:
            raise ValueError(f"Required columns missing in {tsv_path}: {missing}")

        # 型変換とパディング
        df = df.select([
            pl.col("trade_date").cast(pl.Utf8).str.replace_all(r"\D","").str.slice(0, 8),
            pl.col("time").cast(pl.Utf8).str.replace_all(r"\D","").str.zfill(9).str.slice(0, 9),
        ...
        df = df.with_columns(trade_dt.alias("trade_dt")).drop_nulls(subset=["trade_dt"])
    ...
    def process(df: pl.DataFrame) -> pl.DataFrame:
        ...
        cm = df.group_by("trade_date").agg(pl.col("contract_month").min().alias("near_cm"))
        ...
        df = df.sort("trade_dt")
        df = df.with_columns(pl.col("trade_price").shift(1).alias("prev_price"))
        ...
        df = df.with_columns([
            pl.when(pl.col("direction") == 1).then(pl.col("trade_volume")).otherwise(0).alias("buy_volume"),
            pl.when(pl.col("direction") == -1).then(pl.col("trade_volume")).otherwise(0).alias("sell_volume"),
            pl.when(pl.col("direction") == 1).then(1).otherwise(0).alias("buy_tick"),
            pl.when(pl.col("direction") == -1).then(1).otherwise(0).alias("sell_tick"),
        ])
        ...
        q = (
            df.filter((pl.col("price_type") == "N") & (pl.col("trade_volume") > 0)).lazy()
            ...
            .agg([
                ...
                pl.col('trade_volume').std().fill_null(0).alias('vol_size_std'),
                
                # ★追加: その1分間で発生した「最大の約定サイズ」
                pl.col('trade_volume').max().alias('max_trade_size'),
                
                # ★追加: 買い圧力の合計 (Buy Vol - Sell Vol) の純額
                (pl.col('buy_volume').sum() - pl.col('sell_volume').sum()).alias('delta_volume')
            ])
            # 平均約定サイズの算出 (volume / count)
            .with_columns([
                (pl.col('buy_volume') / pl.col('buy_tick_count')).fill_nan(0).fill_null(0).alias('avg_buy_size'),
                (pl.col('sell_volume') / pl.col('sell_tick_count')).fill_nan(0).fill_null(0).alias('avg_sell_size')
            ])
...
class ConvertMT5:
    """MT5 Tick TSV (timestamp, bid, ask) の変換ロジックを管理するクラス"""
    ...
    def read_tick_tsv(tsv_path: str) -> pl.LazyFrame:
        ...
        lf = lf.with_columns(
            pl.col("timestamp").str.strptime(pl.Datetime, format="%Y.%m.%d %H:%M:%S.%3f", strict=False)
            .alias("mt5_ts")
        )
...
def main():
    parser = argparse.ArgumentParser(description="Convert Tick Data (JPX/MT5) to 1s Parquet")
    parser.add_argument("mode", choices=["nk225", "mt5"], help="Conversion mode")
    
    # NK225用: 単一ファイル or リストファイル
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--input", "-i", help="Input TSV file path")
    group.add_argument("--list", "-l", help="Path to text file listing TSVs (NK225 only)")
