...
class BacktestSimulator:
    ...
    def _prepare_market_paths(
        ...
        t_rem_raw = self.data["time_to_closes"][entry_mask].astype(
            np.float32, copy=False
        )

        # minutes_to_close が 0〜1 に正規化された値で流れてきた場合でも
        # 1バー固定にならないように保険をかける
        if t_rem_raw.size == 0:
            act_horizon = np.empty(0, dtype=np.int32)
        elif float(np.nanmax(t_rem_raw)) <= 1.5 and max_h > 1:
            act_horizon = np.full(len(t_rem_raw), max_h, dtype=np.int32)
        else:
            t_rem = np.clip(
