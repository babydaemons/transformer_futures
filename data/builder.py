...
def auto_tune_label_threshold_scale(
    ...
    """ラベル分布（Neutral/Trade）の偏りを抑えるため、Foldごとに threshold_factor を自動調整します。
    ...
    Args:
        ...
        cost_floor (float): 手数料・スリップを考慮した最小閾値フロア。
        min_limit_mode (str): ラベル最小値の適用モード ("fixed", "cost", "max")。
    ...
    logger.info(
        f"Auto-tuned threshold_factor: {base_scale:.3f} -> {best_scale:.3f} (Neutral Ratio: {ratio:.1%})"
    )
    return best_scale
...
def _generate_fold_labels(
    ...
    """
    t_scale = float(cfg.features.label_threshold_scale)
    ...
    min_limit_mode = str(cfg.features.label_min_limit_mode)
