...
def build_fold_dataloaders(
    ...
    Args:
        start_dt (datetime.date): チャンクロードの開始日。
        end_dt (datetime.date): チャンクロードの終了日。
        ...
        builder (DatasetBuilder): Dataset変換用ビルダー。
    ...
    with PerfTimer(logger, f"load_and_compute_features"):
    ...
    c_tr, s_tr, y_tr, p_high_tr, p_low_tr, p_close_tr, ts_tr = builder.prepare_data(
        df_tr
    )
    c_val, s_val, y_val, p_high_val, p_low_val, p_close_val, ts_val = (
        builder.prepare_data(df_v)
    )
    c_test, s_test, y_test, p_high_test, p_low_test, p_close_test, ts_test = (
        builder.prepare_data(df_te)
    )
